...
export class QRPay {
  ...
  public build(): string {
    ...
    const initMethod = QRPay.genFieldData(
      FieldID.INIT_METHOD,
      this.initMethod ?? InitMethod.STATIC,
    );
    ...
    const nation = QRPay.genFieldData(
      FieldID.NATION,
      this.nation ?? Nation.VIETNAM,
    );
    const merchantName = QRPay.genFieldData(
  ...
  public static initVietQR(options: {
    bankBin: string;
    bankNumber: string;
    amount?: string;
    purpose?: string;
    ...
    const qr = new QRPay();
    qr.initMethod = options.amount ? InitMethod.DYNAMIC : InitMethod.STATIC;
